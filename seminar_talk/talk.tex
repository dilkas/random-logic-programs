...
% TODO: I could talk about the constraints behind this
...
\begin{frame}{Independence: $\textcolor{predicate}{\mathrm{influences}}
  ...
  \begin{tikzpicture}
    ...
    \node[draw,ellipse,onslide=<2->{fill=constant!25},right=of friend] (influences) {\textcolor{predicate}{influences}};
    ...
    \draw[-{Latex[length=2mm]}] (person) -- (spont);
    \draw[-{Latex[length=2mm]}] (person) -- (smoke);
    \draw<3->[-{Latex[length=2mm]}] (influences) edge[draw=red,bend left=15] (stress);
    \draw<3->[-{Latex[length=2mm]}] (stress) edge[draw=red,bend left=15] (influences);
