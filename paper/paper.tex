...
will use $i$ and $X$ interchangeably.
...
\begin{itemize}
\item Heads are too restrictive: $\variable{P}(X, Y, X)$ should be allowed.
...
Our definition is slightly more involved because we want to impose some
additional constraints, namely, that the variables in the atom are consecutive 

\begin{definition} % mention: variables are always ordered, constants are not.
  ...
  \end{itemize}
  We define the \variable{predicate}'s \variable{arity} using the
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    \node (P) at (-1, -2) {$\variable{P}(X)$};
    \node[draw,circle,gray,text=black] (and) at (1, -1) {$\land$};
    \node (Q) at (0, -2) {$\variable{Q}(X)$};
    \node (R) at (2, -2) {$\variable{P}(X)$};
    \draw[gray] (or) -- (not);
    \draw[gray] (or) -- (and);
    \draw[gray] (not) -- (P);
    \draw[gray] (and) -- (Q);
...
\begin{example} \label{example:formula}
  Let $\maxNumNodes{} = 8$. Then $\neg\variable{P}(X) \lor (\variable{Q}(X)
  \land \variable{P}(X))$ corresponds to the tree in \cref{fig:example_tree} and
  can be encoded as:
  \begin{alignat*}{8}
    \variable{treeStructure} &= [0, &&0, &&0, &&1, &&2, &&2, &&6, &&7],\\
    \variable{treeValues} &= [{\lor}, &&{\neg}, &&{\land}, \variable{P}(&&X), \variable{Q}(&&X), \variable{P}(&&X), &&\top, &&\top],\\
    \variable{numNodes} &= 6, && && && && && && &&\\
    \variable{numTrees} &= 3. && && && && && && &&
  \end{alignat*}
...
\begin{example}
  Let $\maxArity{} = 2$, $\predicates{} = [\variable{P}, \dots]$, $\arities{}
  = [1, \dots]$, and $X \in \variables{}$. Then the node representing atom
  $\variable{P}(X)$ has:
  \begin{align*}
    \variable{name} &= \variable{P},\\
...
% TODO: define independence of predicates
% Formal definition (here and in the predicate invention paper): two
