...
\begin{algorithm}[p]
  ...
  \For{$(d_1, d_2) \in \getDependencies{$p_1$} \times
    \getDependencies{$p_2$}$ such that $d_1.\predicate = d_2.\predicate$}{
    \lIf{$d_1$.\isDetermined{} {\bf and} $d_2$.\isDetermined{}}{\fail{}}
...
\begin{algorithm}[p]
  ...
  \If{$\exists (d_1, d_2) \in D$ such that $d_1.\isDetermined{}$ {\bf and}
    $d_2.\isDetermined{}$}{
    \Return{\textsc{false}}\;
  }
...
\begin{algorithm}[p]
  ...
  \KwData{a $|\predicates{}| \times |\predicates|$ adjacency matrix $\mathbf{A}$
  with $\{ 0, 1 \}$ as the domain}
  \Fn{\getDependencies{$p$}} {
    ...
    \While{{\normalfont \texttt{true}}}{
      ...
      \For{$d \in D$ {\bf and} $q \in \predicates{}$}{
        ...
        \uElseIf{$\edgeExists$}{
          $D' \gets D' \cup \{ (q, d.\source, d.\target) \}$\;
        }
        \ElseIf{$|\mathbf{A}[q][d.\predicate]| > 1$ {\bf and} $d$.\isDetermined{}}{
          $D' \gets D' \cup \{ (q, q, d.\predicate) \}$\;
        }
      }
      \lIf{$D' = D$}{\Return{$D$}}
      $D \gets D'$\;
...
\begin{algorithm}[t]
  ...
  \Return{\textsc{undefined}}\;
  \caption{Entailment checking for negative cycles}
\end{algorithm}
