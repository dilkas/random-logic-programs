...
\begin{definition}
  ...
  \begin{itemize}
  \item $\arrayd{cstructure}{\maxNumNodes{}}{[0, \maxNumNodes{} - 1]}$ such
    that:
    \begin{itemize}
...
\begin{constraint} \label{constraint:node_types}
  ...
  \begin{align*}
    C = 0 &\iff \variable{values}[i].\variable{name} \in \predicates{} \cup \{ \top \},\\
    C = 1 &\iff \variable{values}[i].\variable{name} = \neg,\\
    C > 1 &\iff \variable{values}[i].\variable{name} \in \{ \land, \lor \},
  \end{align*}
  where $\variable{structure}_{>i}$ denotes array $\variable{structure}$ with positions
  greater than $i$.
\end{constraint}
...
\begin{algorithm}
  ...
  \For{$(d_1, d_2) \in \getDependencies{$p_1$, 0} \times \getDependencies{$p_2$,
    ...
    \If{$d_1$ {\bf is} $\Determined(\_)$ {\bf and} $d_2$ {\bf is}
      ...
      $\Determined(\_)$ {\bf and} $d_1$ {\bf is} $\AlmostDetermined(\_, s, t)$}{
...
\item $|\predicates{}|$, $|\variables{}|$, $|\constants{}|$, $\maxNumNodes{}$,
  the number of additional clauses are in $\{ 1, 2, 4, 8, 16 \}$.
