...
\section{Considering Each Clause Separately}
% TODO: define heads & bodies

\subsection{Heads of Clauses}
...
\subsection{Eliminating Variable Symmetries} \label{sec:variable_symmetry}
...
\begin{definition}
  % TODO: what odes this mean? example?
  Let $N = \maxArity{} \times (\maxNumNodes{} + 1)$. Let
  $\variable{termsPerClause}[N] \in \constants{} \cup \variables{} \cup \{ \Box
  \}$ be a matrix, each row of which is a flattened list of all positions for
  terms in a particular clause.

  Then we can use the $\variable{setsIntsChanneling}$ constraint
  to define $\variable{occurrences}[|\constants{}| + |\variables{}| + 1]$ as a
  matrix of subsets of $\{ 0, \dots, N-1 \}$ such that for all $i = 0, \dots, N
  - 1$, and $t \in \constants{} \cup \variables{} \cup \{ \Box \}$,
  \[
    i \in \variable{occurrences}[t] \quad \iff \quad
    \variable{termsPerClause}[i] = t
  \]
\end{definition}

\begin{definition}
  We define $\arrayd{M}{|\variables{}|}{\{ 0, \dots, N \}}$ such that for $v \in
  \variables{}$,
  \[
    \variable{M}[v] = \begin{cases}
...
\begin{constraint}
  $\variable{M}$ is sorted.
\end{constraint}

\begin{example} % TODO: update the example to cover an entire clause
...
% TODO: define combined arity.
% TODO: define 'gap', 'argument', 'position' or something like that.
...
formula from this section in 985 different scenarios.
...
bodies with $n$ nodes and combined arity $a$. Then $T(1, a)$ is the number of
...
The number of ways to fill $n$ positions with terms can be expressed as
\[
  P(n) = |\constants{}|^n + \sum_{\substack{1 \le k \le |\variables{}|, \\ 0 =
      s_0 < s_1 < \dots < s_k < s_{k+1} = n+1}} \prod_{i=0}^k (|\constants{}| +
  i)^{s_{i+1} - s_i - 1}.
\]
The first term is simply the number of ways to fill all $n$ positions with
