...
\begin{itemize}
\item Construct matrix M for variables that don't participate in the head. Have
  a variable variablesThatParticipateInTheHead that separates the set of
  variables.
...
\section{Eliminating Variable Symmetries}
% TODO: the matrix should include two more columns for bodies

\begin{definition} We define an array \variable{occurrences} of $|\constants{}|
  + |\variables{}|$ sets such that, for all $t \in \constants{} \cup
  \variables{}$, $\variable{occurrences}[t]$ is a subset of $\{ 0, \dots,
  \maxArity{} - 1\}$ that stores the indices of \variable{arguments} at which
  $t$ is located. It is defined by the \variable{setsIntsChanneling} constraint
  equivalent to the following condition: for $i = 0, \dots, \maxArity{} - 1$ and
  $t \in \constants{} \cup \variables{}$,
  \[
...
\begin{definition}
  ...
  \end{align*}
\end{definition}
\footnotetext{This could be any fixed value in the domain.}
...
% TODO: turn this into lexChainLessEq. But before: update the counting section.

\section{Counting Programs}
% TODO: update to account for the new definition of head
...
% TODO: does it work? explain why it works
...
% TODO: describe what parameter values were used to test the model.
