...
% TODO: proofread, run a grammar checker on everything
...
\emph{body} (which is a formula\footnote{Our model supports arbitrarily complex
  bodies of clauses because \textsc{ProbLog} does too. However, one can easily
  restrict our representation of a body to a single disjunction of literals by
  adding a couple of additional constraints.}). A \emph{(logic) program} is a
set of clauses, and a \emph{\textsc{ProbLog} program} is a set of
clause-probability pairs.
...
$\arities{}$, $\variables{}$, and $\constants{}$\footnote{We checked that
  our model agrees with the derived combinatorial formula in up to a thousand
