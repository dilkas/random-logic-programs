...
% TODO (for later)
% 1) First keyword should be the category that I'm submitting for (add a new one)
% 2) Use American English
% 3) Avoid the Definition environment (to save space)
% 4) Make sure that figures and algorithms are placed in good positions
% 5) Check the grammar
...
\begin{abstract}
  ...
  correctness and describe how the parameters of the model affect the empirical
  difficulty of the program generation task.

  \keywords{Probabilistic logic programming \and Statistical relational
...
particular, logic programs and probabilistic logic programs---including
languages such as PRISM \cite{DBLP:conf/ijcai/SatoK97}, ICL
\cite{DBLP:journals/ai/Poole97}, and ProbLog
...
% TODO: Review: I’m not convinced that paragraph 2 follows from paragraph 1. Has
% generation of random SAT problems really solved the problem of establishing
% that one inference algorithm is better than another? I thought it led to the
% opposite–a discovery that randomly generated instances are a different kind of
% problem than real-world benchmarks. For example, a recent SAT competition
% shows now overlap in the top three solvers for the random vs non-random
% tracks.
...
generation of random SAT instances against different input parameters (e.g.,
clause length and total number of variables) to better understand the behaviour
of algorithms and their ability to solve random synthetic problems.
...
clauses with only two literals \cite{DBLP:conf/lpnmr/NamasivayamT09}, or to
clauses of the form $a \gets \neg b$ \cite{DBLP:journals/tocl/WenWSL16},
although some are more expressive, e.g., defining a program only by the
(maximum) number of atoms in the body and the total number of rules
\cite{DBLP:conf/iclp/ZhaoL03}.

In this work, we introduce a constraint model for generating random logic
programs according to a number of user-specified parameters on the structure of
the program. In fact, the same model can generate both probabilistic programs
in the syntax of ProbLog \cite{DBLP:conf/ijcai/RaedtKT07} and
non-probabilistic Prolog programs. A major advantage of our constraint-based
approach is that one can easily add additional constraints to the model. To
demonstrate that, we present a custom constraint with propagation and entailment
algorithms that can ensure predicate independence. We also present a
combinatorial argument for correctness, counting the number of programs that the
model produces for various parameter values. Finally, we show how the model
scales when tasked with producing more complicated programs.

A common restriction put on both logic programs and probabilistic logic programs
in order to ensure that there is a unique answer to every query is known as
\emph{stratification}
...
% TODO: Review: I would have appreciated a definition of 
% predicate independence instead of just a forward pointer to sections 7 and 8.
...
% TODO: Answer to a review: The main benefit of a constraint-based approach is
% the ability to add additional constraints as needed, and to do that
% efficiently (compared to generate-and-test approaches). Specifically, we can
% regulate what parts of the program must be independent, require certain
% formulas to be included, etc.

% TODO: Review: The paper fails to provide explanation of critical parts: for
% example, how is a set of constraints going to be turned into a random logic
% program? We don’t find out until section 9.
...
\mathscr{P}$.
% TODO: Review: The use of disjunction and arbitrary negation in the body is
% also uncommon
...
% TODO: make the first sentence clear
The model supports enforcing predicate independence (see
...
\begin{constraint}
  Clauses are sorted according to a lexicographic increasing order\footnote{The
    exact order is immaterial: we only impose an order to eliminate duplicates
...
This $\variable{predicate}$ variable has a corresponding arity that depends on
the value of $\variable{predicate}$. We can define $\variable{arity} \in [0,
\maxArity{}]$ as the arity of the $\variable{predicate}$ if
$\variable{predicate} \in \predicates{}$ and zero otherwise using the table
constraint \cite{DBLP:conf/cpaior/MairyDL15}. This constraint uses a set of
pairs of the form $(p, a)$, where $p$ ranges over all possible values of
$\variable{predicate}$, and $a$ is either the arity of predicate $p$ or zero.
...
\begin{example} \label{example:formula}
  ...
  \begin{gather*}
    \begin{alignedat}{8}
      ...
      \variable{values} &= [{\lor}, &&{\neg}, &&{\land}, \mathsf{P}(&&X),
      \mathsf{Q}(&&X), \mathsf{P}(&&X), &&\top, &&\top],
    \end{alignedat} \\
    \variable{numNodes} = 6, \quad \variable{numTrees} = 3.
  \end{gather*}
\end{example}
...
% TODO: Response to a review: Symmetry removal is important for two reasons.
% First, if we want to generate all programs that match a certain description,
% we would rather not generate the same program multiple times. Second, symmetry
% removal makes the constraint solving task much simpler by significantly
% reducing the search space that needs to be explored, often turning an
% intractable problem into an easy one.
...
that the constraints described in this section only make sense if $|\variables| >
...
% TODO: Does this section still work if I don't allow duplicates?

\setcounter{footnote}{0}

% TODO: Review: Finally, regarding correctness of the proposal, I don´t
% understand "the combinatorial argument for correctness" result, how does the
% provided analysis show correctness relative to the properties? I was expecting
% a proof that would show the relationship between the formalized constraint and
% the desired property, to chow for instance that if such constraint is taken
% into account in the construction of the program then it is impossible that the
% program does not satisfy the property. Is there something I do not understand
% from this? I find the proposal lacking in this terms, not finding the
% arguments for validity strong enough, unless I misunderstood the analysis.
% Answer: 
% Regarding correctness, we decided against including proofs that a constraint for
% a property ensures the property because the discussed properties are all
% syntactic, and the constraint model itself is a representation of the same
% syntax so the proofs would not be particularly revealing. We can nonetheless add
% them if necessary.
%
% Instead, Section 6 shows correctness in a sense that if two methods generate the
% same numbers (i.e., numbers of programs with certain properties and parameters),
% then they describe the same thing, and we gain confidence that the constraint
% description (which matches the combinatorial description) accurately matches our
% intentions. This method helped us debug many aspects of the model and can be
% particularly revealing when both the model and the combinatorial argument become
% more intricate.
...
% TODO: Review: Section 7 defined predicate (in)dependence. It is done using the
% constraint model, but given a logic program, it could just as well be
% extracted from the logic program (construct body tree from each clause).
% Answer: Motivate why we need this.

% TODO: Answer to a review: All constraints have propagation and entailment
% procedures behind them. When designing a new constraint, unless it can be
% composed of already-existing constraints, designing these algorithms is, in
% fact, completely in the spirit of constraint programming.
...
% TODO: Review: From section 9 it seems that this language has a very special
% handcrafted (heuristic) solution procedure that is not guaranteed to find a
% solution if one exists.
% Answer: Adding a restart policy helps with efficiency when generating larger
% programs, but if we want to find all possible programs, restarts can be
% disabled (and this is likely to only be relevant with low parameter values,
% where the number of possible programs is in thousands rather than millions).
% Note that combinations of parameter values that have no solutions (or are
% unlikely to have solutions) are unlikely to occur in practice (i.e., be of
% interest).
...
In addition, note that our model treats logically equivalent but syntactically
...
% 6. Logic programs typically only have a single conjunction with literals, but
% we can easily adapt the model to handle that.
