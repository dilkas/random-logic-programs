...
\begin{abstract}
  ...
  \keywords{Probabilistic logic programming \and Logic programming \and
    Predicate independence.}
\end{abstract}

% TODO (for later)
% 1) First keyword should be the category that I'm submitting for
...
directly in the syntax of ProbLog \cite{DBLP:conf/ijcai/RaedtKT07} and
non-probabilistic Prolog programs. For generated probabilistic programs to be
valid, we use a custom constraint to detect negative cycles. A major advantage
of our constraint-based approach is that one can easily add additional
constraints to the model. To demonstrate that, we present a custom constraint
with propagation and entailment algorithms that can ensure predicate
independence. We also present a combinatorial argument for correctness, counting
the number of programs that the model produces for various parameter values.
Finally, we show how the model scales when tasked with producing more
complicated programs and identify the relationships between parameter values and
the empirical hardness of the program generation task.
...
\begin{constraint}
  Let $P = \{ h.\variable{predicate} \mid h \in \variable{heads} \}$. Then
  \[
    ...
    \end{cases}
  \]
  where $\variable{nValues}(P)$ counts the number of unique values in $P$.
\end{constraint}
...
\begin{example} \label{example:independence}
  ...
  % TODO: labels on the inside (check if they work)
  \begin{figure}
    \begin{minipage}{.49\textwidth}
      \centering
      \subfloat[The adjacency matrix defined using \cref{def:adjacency_matrix} for
      \cref{example:independence}\label{fig:dependencies_matrix}]{
    ...
    \begin{minipage}{.49\textwidth}
      \centering
      \subfloat[The predicate dependency graph that corresponds to
      \cref{fig:dependencies_matrix}. Dashed edges are undetermined---they may or
...
% although some work has been done in overcoming this limitation
% \cite{DBLP:journals/ijar/BuchmanP17}

Having no negative cycles in the predicate dependency graph is a requirement of
ProbLog that makes the program well-defined \cite{kimmig2009trading}. Ideally,
we would like to design a constraint for negative cycles similar to the
