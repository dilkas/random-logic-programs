...
%programs \citep{DBLP:conf/soict/Dries15} and can be used as a component of
%learning.
...
languages such as PRISM \citep{DBLP:conf/ijcai/SatoK97}, ICL
\citep{DBLP:journals/ai/Poole97}, and ProbLog
...
data sets
\citep{DBLP:journals/tplp/KimmigDRCR11,DBLP:conf/ecai/BruynoogheMKGVJR10,DBLP:conf/ijcai/VlasselaerBKMR15},
...
clauses with only two literals \citep{DBLP:conf/lpnmr/NamasivayamT09}, or to
...
directly in the syntax of ProbLog \citep{DBLP:conf/ijcai/RaedtKT07} and
...
Let
  ...
  \]
  Then $\variable{nValues}(P) = |\predicates{}|$ if $\variable{count}(\Box, P) =
  0$ and $|\predicates{}| + 1$ otherwise, where $\variable{nValues}(P)$ counts
  the number of unique values in $P$.
\end{constraint}
...
% \citep{DBLP:journals/ijar/BuchmanP17}

Having no negative cycles in the predicate dependency graph is a requirement of
ProbLog that makes the program well-defined \citep{kimmig2009trading}. Ideally,
...
Sciences Research Council (grant EP/S023208/1). The authors would like to thank
Fazl Barez for his comments.
