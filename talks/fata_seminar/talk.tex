...
% TODO: maybe add a specific date
...
\begin{frame}{Probabilistic Inference: Reasoning by Hand}
  \begin{minipage}[t][0.5\textheight]{\textwidth}
    ...
    \only<1>{
      $\begin{aligned}[t]
        \Pr[\textcolor{predicate}{\mathtt{cancer}}(\textcolor{constant}{\mathit{mary}})]
        &=
        \Pr[\textcolor{predicate}{\mathtt{cancer\_spont}}(\textcolor{constant}{\mathit{mary}})]
        \\
        &\oplus
        \Pr[\textcolor{predicate}{\mathtt{smokes}}(\textcolor{constant}{\mathit{mary}})]
        \\
        &\times
        \Pr[\textcolor{predicate}{\mathtt{cancer\_smoke}}(\textcolor{constant}{\mathit{mary}})]
      \end{aligned}$
    }
    \only<2->{
      $\Pr[\textcolor{predicate}{\mathtt{cancer}}(\textcolor{constant}{\mathit{mary}})]
    ...
    \only<3>{
      $\begin{aligned}[t]
        \Pr[\textcolor{predicate}{\mathtt{smokes}}(\textcolor{constant}{\mathit{mary}})]
        &=
        \Pr[\textcolor{predicate}{\mathtt{stress}}(\textcolor{constant}{\mathit{mary}})]
        \\
        &\oplus
        \Pr[\textcolor{predicate}{\mathtt{smokes}}(\textcolor{constant}{\mathit{albert}})]
        \\
        &\times
        \Pr[\textcolor{predicate}{\mathtt{influences}}(\textcolor{constant}{\mathit{albert}},
    ...
    \only<5->{
      $\Pr[\textcolor{predicate}{\mathtt{smokes}}(\textcolor{constant}{\mathit{albert}})]
      =
      \Pr[\textcolor{predicate}{\mathtt{stress}}(\textcolor{constant}{\mathit{albert}})]
...
\begin{frame}{Probabilistic Inference: Probabilities of Worlds}
  ...
  \onslide<2->{
    \[
      \textcolor{predicate}{\mathtt{cancer}}(\textcolor{constant}{\mathit{mary}})
      = \alt<5>{\alert{\bot}}{\top}
    \]
    \[
      \Pr(\mathit{world}) =
  ...
  \begin{columns}
    \hspace*{-0.7cm}\begin{column}{0.75\textwidth}
      ...
      \textcolor<5->{probability}{0.3} \prob &\textcolor<2->{predicate}{\mathtt{cancer\_smoke}}(\textcolor<3->{variable}{\mathsf{P}}) \ifff \textcolor<2->{predicate}{\mathtt{person}}(\textcolor<3->{variable}{\mathsf{P}}). \\
                                         ...
                                         &\textcolor<2->{predicate}{\mathtt{person}}(\textcolor<4->{constant}{\mathit{mary}}). \\
                                         &\textcolor<2->{predicate}{\mathtt{person}}(\textcolor<4->{constant}{\mathit{albert}}). \\
                                         &\textcolor<2->{predicate}{\mathtt{friend}}(\textcolor<4->{constant}{\mathit{albert}}, \textcolor<4->{constant}{\mathit{mary}}).
        \end{align*}
      \end{empheq}
    \end{column}
...
\begin{frame}{Formulas As Trees}
  \begin{columns}
    \begin{column}{0.37\textwidth}
      ...
      \begin{itemize}
      \item<6-> $\structure{\mathbf{s}}$ is sorted
      \item<6-> $\structure{\mathbf{s}_i \ne i} \implies \structure{\mathbf{v}_i \ne \top}$
      \end{itemize}
    \end{column}
    \begin{column}{0.63\textwidth}
      ...
      \onslide<3->{
        \begin{tabular}{c c|C|C|C|C|C|D}
          \cline{2-8}
          $\structure{\mathbf{s}}\colon$ & \alert<5->{0} & 0 & 0 & 1 & 2 & 2 & 6 \\
          \cline{2-8}
          $\structure{\mathbf{v}}\colon$ & $\lor$ & $\neg$ & $\land$ & $\textcolor{predicate}{\mathtt{p}}(\textcolor{variable}{\mathsf{X}})$ & $\textcolor{predicate}{\mathtt{q}}(\textcolor{variable}{\mathsf{X}})$ & $\textcolor{predicate}{\mathtt{r}}(\textcolor{variable}{\mathsf{X}})$ & $\top$ \\
          \cline{2-8}
          \onslide<8->{$\structure{\mathbf{c}}\colon$ & 2 & 1 & 2 & 0 & 0 & 0 & 0 \\ \cline{2-8}}
        \end{tabular}
      }
      \begin{itemize}
      \item<4-> $\structure{\mathbf{s}}$ is a forest with $\structure{T = 2}$ trees
      \item<7-> length \structure{$L = 7$}
      \item<7-> number of nodes \structure{$N \coloneqq L - T + 1 = 6$}
      \item<7-> for \structure{$i = 1, \dots, L - 1$},
        \begin{itemize}
        \item if \structure{$i < N$}, then \structure{$\mathbf{s}_i < i$}
        \item else \structure{$\mathbf{s}_i = i$} and \structure{$\mathbf{v}_i = \top$}
        \end{itemize}
...
\begin{frame}{Stratification and Negative Cycles}
  ...
  \begin{columns}
    \begin{column}{0.4\textwidth}
      ...
      \begin{itemize}
      \item<3-> \structure{Stratum 1}: predicates with no negation
      \item<3-> \structure{Stratum 2}: new predicates s.t. negative literals refer to
        predicates in \structure{Stratum~1}
      \end{itemize}
      \vspace{5em}
    \end{column}
...
\begin{frame}{Predicate Independence: $\textcolor{predicate}{\mathrm{friend}} \independent
...
% 1) say something about heuristics
% 2) conditional independence?
% 3) how is a predicate with its arguments represented?
% 4) a full(er) example of independence propagation
% 5) could talk about what algorithms are behind the negative cycle detection
% 6) for the bigraph talk, I had 28 slides full info and it still wasn't
% enough. Maybe I should aim for at least 30. Right now I have 26.
% 7) more constraints
% 8) more motivation and related work
